...
def find_neighbors_for_target(
    ...
    Returns a DataFrame with columns:
      target_mmsi, t_unix, lat_deg, lon_deg, neighbor_mmsi, neighbor_dist_km, n_neighbors
    ...
    for t, df_tgt_t in df_target.groupby("t_unix"):
        ...
        if df_oth_t.empty:
            for _, r in df_tgt_t.iterrows():
                out_rows.append({
                    "target_mmsi": int(r.mmsi), "t_unix": int(t),
                    "lat_deg": float(r.lat_deg), "lon_deg": float(r.lon_deg),
                    "neighbor_mmsi": None, "neighbor_dist_km": None,
                    "n_neighbors": 0
                })
            continue
